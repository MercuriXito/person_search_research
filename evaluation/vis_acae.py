...
class FeatureMapVisualizer:
    ...
    def _search_ind_in_boxes(self, boxes, target_box):
    ...
    def vis_feature_map(
        ...
        for qind, gind, qbox, gbox in \
            ...
            for bidx in range(len(target_items)):
                ...
                # search target inds
                ctx_features[bidx][ctx_notation][target_ind].sum().backward(retain_graph=True)  # BP
                ...
                saved_img_path = os.path.join(self.save_root, saved_img_name)
                print(cv2.imwrite(saved_img_path, oimage))
...
class CTXAttnWeightsVisualizer(CTXFeatureMapVisualizer):
    ...
    def draw_attn_weights(
        ...
        for qind, gind, qbox, gbox in \
            ...
            attn_inds = [[2, 3], [0, 1]]  # self and cross-attn weights for each samples.
            for bidx, (attn_ind, target_ind, item, boxes_img) in \
                ...
                saved_img_path = os.path.join(self.save_root, saved_img_name)
                print(cv2.imwrite(saved_img_path, image))
